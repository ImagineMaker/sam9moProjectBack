...
i1_appkey    = "PSmcjeP3zDV3CcYPJlBVN0o4e2G2nnm7Q3c5"
i1_appsecret = "Eil8nzCeczsNwLjDkdzJRKo9gQrQpFZbNm8zT861ET8ihEGwNoBmRyIIDHhBMpDvF1Vffe7YlwnwuS60FHcbG3Cn96tpUpvs52RzHbseXV8AhczfBjInwYffFMZRAld2aEn6VmdMUsg0LL/LMojd82WozKpe6EeWql0yAz1oLY80Kl0JDSc="


# websocket key
i1_approval_key = get_approval(i1_appkey, i1_appsecret)

print("approval1_key [%s]" % (i1_approval_key))


# 10개종목 돌리기
for i in range(0, 40):
    send_kospi = {
    "header": {"approval_key": i1_approval_key, "custtype": "P", "tr_type": "1", "content-type": "utf-8"},
    "body": { "input": {"tr_id": "H0STCNT0",  # API명
                        "tr_key": kospi_stock[i]  # 종목번호
                       }
            }
    }
    kospi_stock_list.append(send_kospi)
...
def on_message(ws, data):
    if data[0] in ['0', '1']:  # 시세데이터가 아닌경우
        ...
        if len(d1) >= 4:
            ...
            # print(pdbind(result).to_json(orient='records'))
# --------------------------------------------------- #메세지 보내는 구간 
            msg = pdbind(result)
